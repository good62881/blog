...
export default {
	...
	computed: {
		dayList:function() {
			//获取日期数组
			var that=this;
			var fullDay = new Date(that.year,that.month+1,0).getDate(), //当月总天数
				startWeek = new Date(that.year,that.month,1).getDay(), //当月第一天是周几
				total = (fullDay+startWeek)%7 == 0 ? (fullDay+startWeek) : fullDay+startWeek+(7-(fullDay+startWeek)%7),//元素总个数
				lastMonthDay = new Date(that.year,that.month,0).getDate(), //上月最后一天
				arr = [];
			for(var i = 0; i < total; i++){
				if(i<startWeek){
					arr.push({val:lastMonthDay-startWeek+1+i})
				}else if(i<(startWeek+fullDay)){
					arr.push({val:i+1-startWeek,date:that.year+'/'+(that.month+1)+'/'+(i+1-startWeek)})
				}else{
			...
			if (that.dayData) {
				arr.forEach(function(v,i,a) {
					that.dayData[v.date] && (a[i].num=that.dayData[v.date])
				})
			}
			//绘制日历
			var arrFilter = [];
		...
		monthList:function() {
			...
			arr.forEach(function(v,i,a) {
				a[i].date=that.year+'/'+(v.index+1)
			});
			//填充数据
			if (that.monthData) {
				arr.forEach(function(v,i,a) {
					that.monthData[that.year+'/'+(v.index+1)] && (a[i].num=that.monthData[that.year+'/'+(v.index+1)])
				})
			};
			//绘制月份
			var arrFilter = [];
		...
		yearList:function() {
			...
			var arr = [];
			for(var i=0;i<9;i++){
				arr.push({val:that.year-4+i});
			};
			//填充数据
			if (that.yearData) {
				arr.forEach(function(v,i,a) {
					that.yearData[v.val] && (a[i].num=that.yearData[v.val])
				})
			}
			//绘制年份
			var arrFilter = [];
